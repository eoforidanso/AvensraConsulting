...
| 2.7 | **Founder photograph** | The About page now features Nana Ama Setorwofia (Founder & Principal Consultant), but no photograph was supplied. `src/components/FounderPortrait.tsx` is a brand-consistent monogram standing in for one. Supply a square portrait as `public/founder.jpg` and swap the `<svg>` for `next/image` — the component's comment gives the exact steps. A professional headshot matters more here than on most pages: this is the one place the consultancy is represented by a person rather than a brand. |
...
| 6.5 | ~~Executive Career Positioning™ not published~~ **PUBLISHED** | Approved offer supplied in chat 2026-08-30 and published at `/executive-career-positioning`, linked from the header, footer and `/consulting`. This also replaced the four-pillar "Strategy & Business Alignment" framing on `/consulting` and the homepage — that was drawn from decorative bullet points on the artwork sheet, not a specified service catalogue; the approved offer is now Executive Career Positioning™ + Business-to-People Advisory Support. Flag if the four-pillar content should have stayed alongside it — it's intact in git history (commit before 2026-08-30) and easy to restore or re-add. |
